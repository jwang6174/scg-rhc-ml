...
def get_flat_lines(waveform, threshold, min_duration, sampling_rate):
  ...
  Args:
    ...
    threshold (float): Amplitude threshold to be considered flat.
    min_duration (float): Minimum duration (seconds) to be considered flat.
    sampling_rate (int): Waveform sampling rate (Hz).
...
def in_rhc_range(waveform, min_RHC):
  """
  Args:
    waveform (list[float]): Signal time series.
...
def has_noise(
    waveform, flat_threshold, flat_min_duration, 
    straight_threshold, min_RHC, sampling_rate):
  """
  Args:
    waveform (list[float]): Signal time series.
